...
本指南将帮助你在安装了宝塔面板的服务器上部署 NoteDoc 协作文档平台。
...
在 `/www/wwwroot/notedoc` 目录下创建 `docker-compose.yml` 文件：
...
services:
  notedoc:
    ...
    environment:
      ...
      DATABASE_URL: 'postgresql://notedoc:your-db-password@db:5432/notedoc?schema=public'
...
```bash
# 备份数据库
docker exec notedoc_db pg_dump -U notedoc notedoc > /www/backup/notedoc_db_$(date +%Y%m%d).sql

# 备份文件存储
docker run --rm -v notedoc_notedoc_data:/data -v /www/backup:/backup alpine tar czf /backup/notedoc_files_$(date +%Y%m%d).tar.gz -C /data .
```
...
编辑 `docker-compose.yml`，在 `notedoc` 服务的 `environment` 中添加：
...
NoteDoc 支持 Typesense 全文搜索，可以提升搜索性能。

## 支持与帮助

- 官方文档：https://notedoc.com/docs
- GitHub 仓库：https://github.com/notedoc/notedoc
- 问题反馈：https://github.com/notedoc/notedoc/issues

---

**部署完成！** 现在你可以开始使用 NoteDoc 进行团队协作了。
